...
suite "random stress test":
  ...
  block:
    ...
    test "timed random tree iterations":
      ...
      let finish = cpuTime()
      checkpoint "CPU time ", finish - start, "s for ", BB_ITERATIONS, " loops, averaging ",
                 (finish - start) / toFloat(BB_ITERATIONS), "s per loop"
      checkpoint "random q: ", q_ins, " inserts, ", q_del, " deletes, ",
